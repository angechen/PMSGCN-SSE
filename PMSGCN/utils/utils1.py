...
def get_uvd2xyz(uvd, gt_3D, cam):
    ...
    """
    N, T, V,_ = uvd.size()  #256,1,17 ;uvd:[256,1,17,3]


    dec_out_all = uvd.view(-1, T, V, 3).clone()  # [256*1*17*3].clone
    root = gt_3D[:, :, 0, :].unsqueeze(-2).repeat(1, 1, V, 1).clone()# [256*1*17*3],[256,1,3].unsque=[256,1,1,3].repeat(1,1,17,3)=[256,1,17,3]
    enc_in_all = uvd[:, :, :, :2].view(-1, T, V, 2).clone()  # N*T*V*2,[256,1,17,3][2]=[256,1,17,2].view(-1,1,17,2)=[256,1,17,2]

    cam_f_all = cam[..., :2].view(-1,1,1,2).repeat(1,T,V,1) #[256,9][2]=[256,2].view=[256,1,1,2].repeat=[256,1,17,2]
    cam_c_all = cam[..., 2:4].view(-1,1,1,2).repeat(1,T,V,1)#[256,9][2:4]=[256,2].view=[256,1,1,2].repeat=[256,1,17,2]

    # change to global
    z_global = dec_out_all[:, :, :, 2]# N*T*V[256*1*17*3][2]=[256,1,17]
    z_global[:, :, 0] = root[:, :, 0, 2] #[256,1,17,3][0,2]=[256,1]
    z_global[:, :, 1:] = dec_out_all[:, :, 1:, 2] + root[:, :, 1:, 2]  # N*T*V [256,1,16]+[256,1,16]=[256,1,16]
    z_global = z_global.unsqueeze(-1)  # N*T*V*1[256,1,17].unsqu=[256,1,17,1]
    
    uv = enc_in_all - cam_c_all  # N*T*V*2[256,1,17,2]
    xy = uv * z_global.repeat(1, 1, 1, 2) / cam_f_all  #[256,1,17,2]/  =[256,1,17,2]
    xyz_global = torch.cat((xy, z_global), -1)  #[256,1,17,3]
    xyz_offset = (xyz_global - xyz_global[:, :, 0, :].unsqueeze(-2).repeat(1, 1, V, 1))#[256,1,3].unsqu=[256,1,1,3].repeat=[256,1,17,3]
...
def print_error_action(action_error_sum, show_protocol2=False):
    ...
    if show_protocol2:
        ...
        print("{0:=^12} {1:=^10} {2:=^8}".format("Action", "p#1 mm", "p#2 mm")) #===Action=== ==p#1 mm== =p#2 mm=
        for action,value in action_error_sum.items():  #'Directions'   {'p1':AccumLoss, 'p2':AccumLoss}
            print("{0:<12} ".format(action), end="")
            for j in range(1,3): #1, 2

                mean_error_each['p'+str(j)] = action_error_sum[action]['p'+str(j)].avg * 1000.0 #p1, p2
...
def define_error_list(actions):
    ...
    """
    error_sum = {}
    error_sum.update({actions[i]: {'p1':AccumLoss(), 'p2':AccumLoss(), 'boneLengthError':AccumLoss(), 'boneAngleError':AccumLoss(), 'neighBoneAngleError':AccumLoss(), 'twoStepNeighBoneAngleError':AccumLoss(), 'threeStepNeighBoneAngleError': AccumLoss(), 'fourStepNeighBoneAngleError': AccumLoss(), 'fiveStepNeighBoneAngleError': AccumLoss(), 'sixStepNeighBoneAngleError': AccumLoss(), 'sevenStepNeighBoneAngleError': AccumLoss()} for i in range(len(actions))})
    return error_sum
