    ...
    def __init__(self, path, opt, remove_static_joints=True):
        ...
        for cameras in self._cameras.values():
            for i, cam in enumerate(cameras):
                ...
                cam['intrinsic_matrix'] = np.array([[cam['focal_length'][0], 0, cam['center'][0]],
        ...
        for subject, actions in self._data.items():
            for action_name, multimodeData in actions.items():
                parentNodes = self._skeleton._parents.tolist()
                parentNodes[0] = 0
                multimodeData['boneVector'] = multimodeData['positions'] - multimodeData['positions'][:, parentNodes, :]
                multimodeData['boneLength'] = np.linalg.norm(multimodeData['boneVector'], axis=2)
                multimodeData['boneLengthSum'] = np.sum(multimodeData['boneLength'], axis=1)
