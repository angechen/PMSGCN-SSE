...
from nets.PMS_gcn import Model
...
# load model
model['pms_gcn'] = Model(opt).cuda()  #define skeleton graph and network layers
...
# Reload model
stgcn_dict = model['pms_gcn'].state_dict()
if opt.stgcn_reload == 1:
    ...
    model['pms_gcn'].load_state_dict(stgcn_dict)
...
#training process
for epoch in range(1, opt.nepoch):   #200
    ...
    if opt.pro_test == 1:
        ...
        if opt.save_model and data_threshold < opt.previous_best_threshold:
            opt.previous_pms_gcn_name = save_model(opt.previous_pms_gcn_name, opt.save_dir, epoch, opt.save_out_type, data_threshold, model['pms_gcn'], 'pms_gcn')
